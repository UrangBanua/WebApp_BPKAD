...
type
  TfrmHtmlTugasFungsi = class(TUniFrame)
    UniPanel1: TUniPanel;
    upModif: TUniPanel;
    ubtModif: TUniBitBtn;
    ubtSave: TUniBitBtn;
...
procedure TfrmHtmlTugasFungsi.ubtModifClick(Sender: TObject);
begin
...
procedure TfrmHtmlTugasFungsi.ubtSaveClick(Sender: TObject);
begin
...
procedure TfrmHtmlTugasFungsi.UniFrameBeforeReady(Sender: TObject);
begin
    qHtmlData.Close;
    qHtmlData.Open('SELECT * FROM webapp_BPKAD.dbo.tbl_HTML WHERE '+
                          'Category =' + QuotedStr('Menu') + ' AND '+
                          'Object =' + QuotedStr('Tugas dan Fungsi'));
    //qHtmlData.Open;
...
initialization
  RegisterClass(TfrmHtmlTugasFungsi);
