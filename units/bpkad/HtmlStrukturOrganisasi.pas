...
type
  TfrmHtmlStrukturOrganisasi = class(TUniFrame)
    UniPanel1: TUniPanel;
    upModif: TUniPanel;
    ubtModif: TUniBitBtn;
    ubtSave: TUniBitBtn;
...
procedure TfrmHtmlStrukturOrganisasi.ubtModifClick(Sender: TObject);
begin
...
procedure TfrmHtmlStrukturOrganisasi.ubtSaveClick(Sender: TObject);
begin
...
procedure TfrmHtmlStrukturOrganisasi.UniFrameBeforeReady(Sender: TObject);
begin
    qHtmlData.Close;
    qHtmlData.Open('SELECT * FROM webapp_BPKAD.dbo.tbl_HTML WHERE '+
                          'Category =' + QuotedStr('Menu') + ' AND '+
                          'Object =' + QuotedStr('Struktur Organisasi'));
    //qHtmlData.Open;
...
initialization
  RegisterClass(TfrmHtmlStrukturOrganisasi);
